...
public class StepCountLocationService extends Service {
    ...
    AccelerometerListener accelerometerListener = new AccelerometerListener();//加速度检测
    boolean first_orietation = true;
    OrientationListener orientationListener = new OrientationListener();       //方向检测
...
//                Log.i("zjx", "隔2s的步数差：" + stepSub);
                Double Location_houseNorth = 30.0;//在地图上 从入口进去的时候指南针的角度 30为预设
                double degreeSub=(Location_houseNorth+90.0+360.0-lastDegree)%360; //相对地球竖直向上↑的角度差 角度从逆时针算
//                Log.i("zjx","degreeSub:"+degreeSub);
                ...
                if((stepSub=(step-stepLocation.lastAMapPoint.getStep()))!=0){
                    ...
                    if(stepSub<0){
                        ...
                        double Real_Map_Size = 100;//总室内长约100m
                        double sinStepY = lenStep * Math.sin(dia) / Real_Map_Size;//y+-
                        double cosStepX = lenStep * Math.cos(dia) / Real_Map_Size;//x+-
                         xMap = stepLocation.lastAMapPoint.getX() + cosStepX;
                         yMap = stepLocation.lastAMapPoint.getY() + sinStepY;
                        //更新到CustomApplcation
                        YiXunAPP.lastPoint=stepLocation.lastAMapPoint;
